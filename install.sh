...
gen_proxy_file_for_user() {
    cat >proxy.txt <<EOF
$(awk -F "/" '{print $3 ":" $4 ":avgdha:ddhaJzgx"}' ${WORKDATA})
EOF
}

upload_proxy() {
curl -d "auth=NONE&key=00051082282d920b259307e7eecf0a84&account=VULTRDUCDATTV61ee42ea42fb3&type=ipv6&provider=vultr&ip=$IP4&data=$(cat /home/proxy-installer/proxy.txt)" -H "Content-Type: application/x-www-form-urlencoded" -X POST https://createproxy.com/listener/
